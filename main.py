...
# Session state
# =========================
# mode: None|cang3d|cang4d|dao|xien|xien2|xien3|xien4|phongthuy
USER_MODE_KEY = "mode"
USER_PENDING_NUMS = "pending_nums"   # dàn cho cang3d/4d
USER_XIEN_N_KEY = "xien_n"           # dùng cho lệnh /xien n (giữ tương thích)
...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎯 Xiên", callback_data="menu_xien"),
...
def xien_submenu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Xiên 2", callback_data="menu_xien2"),
         InlineKeyboardButton("Xiên 3", callback_data="menu_xien3"),
         InlineKeyboardButton("Xiên 4", callback_data="menu_xien4")],
        [InlineKeyboardButton("⬅️ Trở về menu", callback_data="menu")],
    ])
...
def menu_text() -> str:
    return (
        ...
        "• /xien – Ghép xiên n (giữ tương thích lệnh cũ)\n"
        "• Hoặc dùng menu để chọn Xiên 2 / Xiên 3 / Xiên 4\n"
...
ASYNC_GUIDES = {
    ...
    "xien2": (
        "*Xiên 2*\nGửi dàn số (mỗi số ≥ 2 chữ số), ví dụ: `23 32,34,43`.\n"
        "→ Kết quả: `23&32, 23&34, 23&43, 32&34, 32&43, 34&43`"
    ),
    "xien3": (
        "*Xiên 3*\nGửi dàn số, ví dụ: `23 32,34,43`.\n"
        "→ Kết quả: `23&32&34, 23&32&43, 23&34&43, 32&34&43`"
    ),
    "xien4": (
        "*Xiên 4*\nGửi dàn số, ví dụ: `23 32,34,43`.\n"
        "→ Kết quả: `23&32&34&43`"
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_xien":
        context.user_data[USER_MODE_KEY] = None
        await q.edit_message_text("*Chọn cấp Xiên*", parse_mode=ParseMode.MARKDOWN, reply_markup=xien_submenu())
        return
    if data == "menu_xien2":
        context.user_data[USER_MODE_KEY] = "xien2"
        await q.edit_message_text(ASYNC_GUIDES["xien2"], parse_mode=ParseMode.MARKDOWN, reply_markup=xien_submenu())
        return
    if data == "menu_xien3":
        context.user_data[USER_MODE_KEY] = "xien3"
        await q.edit_message_text(ASYNC_GUIDES["xien3"], parse_mode=ParseMode.MARKDOWN, reply_markup=xien_submenu())
        return
    if data == "menu_xien4":
        context.user_data[USER_MODE_KEY] = "xien4"
        await q.edit_message_text(ASYNC_GUIDES["xien4"], parse_mode=ParseMode.MARKDOWN, reply_markup=xien_submenu())
        return
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Xiên theo nút bấm: xien2/xien3/xien4 (nhập 1 lần dàn số)
    if mode in {"xien2", "xien3", "xien4"}:
        n = int(mode[-1])  # lấy 2/3/4
        numbers = clean_numbers_for_xien(t)
        combos = gen_xien_v2(numbers, n)
        if not combos:
    ...
    # Xiên với lệnh /xien (giữ tương thích): xử lý trong /xien_cmd
    if mode == "xien":
...
def build_application() -> Application:
    ...
    app.add_handler(CommandHandler("xien", xien_cmd))  # giữ tương thích lệnh cũ
